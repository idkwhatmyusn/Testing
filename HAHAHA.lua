...
-- =========================================================
-- ===============  SHARED COPY AVATAR SYSTEM ==============
-- =========================================================
-- SYNC VIA TELEGRAM WEBHOOK - ALL SCRIPT USERS SEE CHANGES
...
-- ========== TELEGRAM WEBHOOK CONFIG (HARDCODED) ==========
_G.AVATAR_TELEGRAM_BOT_TOKEN = "8258499979:AAG-kp5FQu1vILzuyFOVPxJPf-kAIIPljqo"
_G.AVATAR_TELEGRAM_CHAT_ID = "7671709991"

_G.AvatarWebhookConfig = {
    Enabled = true,
    BotToken = _G.AVATAR_TELEGRAM_BOT_TOKEN,
    ChatID = _G.AVATAR_TELEGRAM_CHAT_ID,
    SyncInterval = 3, -- Check webhook every 3 seconds
    LastMessageId = 0 -- Track last processed message
}
...
task.spawn(function()
    while task.wait(3) do
        ...
        if status == "false" then
            ForceKick("⚠️ Script NKZ Telah Dinonaktifkan Oleh Developer, Mohon Tunggu Informasi Lebih lanjut")
            break
...
task.spawn(function()
    ...
    -----------------------------------------------------
    --   ANTI AFK LEVEL-5 (SERVER INTERNAL SIGNAL)
    -----------------------------------------------------

    -- server anti-AFK biasanya cek:
    -- - perubahan posisi kecil
    -- - perubahan rotation kecil
    -- - humanoid state aktif
    -- - aktivitas camera

    local micro = 0.0005
    local rot = 0.0007

    -- jalankan tiap 15 detik (aman)
    while task.wait(15) do
        pcall(function()

            -- 1️⃣ micro-move (server lihat player bergerak)
            hrp.CFrame = hrp.CFrame * CFrame.new(0, micro, 0)

            -- 2️⃣ micro-rotate (server deteksi orientasi berubah)
            hrp.CFrame = hrp.CFrame * CFrame.Angles(0, rot, 0)

            -- 3️⃣ paksa humanoid dalam state aktif
            hum:ChangeState(Enum.HumanoidStateType.RunningNoPhysics)

            -- 4️⃣ ubah WalkSpeed & HipHeight sangat kecil (server baca sebagai aktivitas)
            hum.WalkSpeed = hum.WalkSpeed + 0.0001
            hum.HipHeight = hum.HipHeight + 0.0001

            -- 5️⃣ camera heartbeat trick (anti-AFK paling kuat)
            workspace.CurrentCamera.CFrame =
                workspace.CurrentCamera.CFrame * CFrame.new(0, 0, 0.0005)
        end)
    end
end)
...
_G.SharedAvatarData = {} -- [UserId] = {copiedUserId, timestamp, username}
...
-- Send update to Telegram webhook
local function sendAvatarUpdateToWebhook(action, userId, copiedUserId, username)
    if not _G.AvatarWebhookConfig.Enabled then return false end
    if not _G.AvatarWebhookConfig.BotToken or _G.AvatarWebhookConfig.BotToken == "" then return false end
    if not _G.AvatarWebhookConfig.ChatID or _G.AvatarWebhookConfig.ChatID == "" then return false end
    ...
    local message = string.format(
        ...
        username or "Unknown",
        tostring(userId),
        tostring(copiedUserId or "Reset"),
        os.date("%H:%M:%S"),
        game.PlaceId
    )
    
    local url = string.format("https://api.telegram.org/bot%s/sendMessage", _G.AvatarWebhookConfig.BotToken)
    local payload = {
        chat_id = _G.AvatarWebhookConfig.ChatID,
        text = message,
        parse_mode = "Markdown"
    }
    ...
    if ok and type(res) == "table" and (res.Success or (res.StatusCode and res.StatusCode == 200)) then
        print("[Avatar Sync] Broadcast:", action, username, "→", copiedUserId or "Reset")
        return true
    end
    return false
end
...
local function getWebhookUpdates()
    if not _G.AvatarWebhookConfig.BotToken or _G.AvatarWebhookConfig.BotToken == "" then return nil end
    if not _G.AvatarWebhookConfig.ChatID or _G.AvatarWebhookConfig.ChatID == "" then return nil end
    
    local url = string.format(
        "https://api.telegram.org/bot%s/getUpdates?chat_id=%s&offset=%d&limit=10",
        _G.AvatarWebhookConfig.BotToken,
        _G.AvatarWebhookConfig.ChatID,
        _G.AvatarWebhookConfig.LastMessageId + 1
    )
    ...
    if not success or not data or not data.ok or not data.result then return nil end
    
    return data.result
end

-- Parse webhook message for avatar data
local function parseAvatarMessage(messageText)
    if not messageText or messageText == "" then return nil end
    
    -- Parse message format: [AVATAR SYNC]\nAction: UPDATE\nUser: username (userId)\nCopied: copiedUserId
    local action = messageText:match("Action:%s*(%w+)")
    local userId = messageText:match("User:.-%((%d+)%)")
    local username = messageText:match("User:%s*([^%(]+)%s*%(")
    local copiedUserId = messageText:match("Copied:%s*(%d+)")
    
    if not action or not userId then return nil end
    
    return {
        action = action,
        userId = tonumber(userId),
        username = username and username:gsub("%s+$", "") or "Unknown",
        copiedUserId = copiedUserId and tonumber(copiedUserId) or nil,
        timestamp = tick()
    }
end

-- =========================================================
-- =============== AVATAR COPY FUNCTIONS ===================
-- =========================================================
...
local function copyAvatarThroughDescription(userId, targetPlayer)
...
-- =========================================================
-- ============= SHARED VISUAL APPLICATION (FIXED) =========
-- =========================================================

local function applySharedAvatar(targetPlayer, copiedUserId)
    if not targetPlayer or not targetPlayer.Character then return end
    
    -- CRITICAL FIX: Skip if target is LocalPlayer (avoid double application)
    if targetPlayer == LocalPlayer then 
        return 
    end
    
    pcall(function()
        copyAvatarThroughDescription(copiedUserId, targetPlayer)
        print("[Shared Avatar] Applied avatar", copiedUserId, "to", targetPlayer.Name, "(from webhook)")
    end)
end

local function resetSharedAvatar(targetPlayer)
    if not targetPlayer or not targetPlayer.Character then return end
    
    -- CRITICAL FIX: Skip if target is LocalPlayer
    if targetPlayer == LocalPlayer then 
        return 
    end
    ...
    pcall(function()
        ...
        if humanoid then
            ...
            humanoid:ApplyDescriptionClientServer(defaultDesc)
            print("[Shared Avatar] Reset", targetPlayer.Name, "to default (from webhook)")
        end
    end)
end

-- =========================================================
-- =============== WEBHOOK SYNC SYSTEM (FIXED) =============
-- =========================================================

local function processWebhookUpdates()
    local updates = getWebhookUpdates()
    if not updates or #updates == 0 then return end
    
    for _, update in ipairs(updates) do
        -- Update last message ID
        if update.update_id then
            _G.AvatarWebhookConfig.LastMessageId = math.max(
                _G.AvatarWebhookConfig.LastMessageId,
                update.update_id
            )
        end
        ...
        if update.message and update.message.text then
            local avatarData = parseAvatarMessage(update.message.text)
            
            if avatarData then
                local targetUserId = tostring(avatarData.userId)
                
                -- Update shared data
                if avatarData.action == "UPDATE" and avatarData.copiedUserId then
                    _G.SharedAvatarData[targetUserId] = {
                        copiedUserId = avatarData.copiedUserId,
                        username = avatarData.username,
                        timestamp = avatarData.timestamp
                    }
                    
                    -- Apply to target player ONLY if NOT LocalPlayer
                    for _, player in ipairs(Players:GetPlayers()) do
                        if tostring(player.UserId) == targetUserId and player ~= LocalPlayer then
                            applySharedAvatar(player, avatarData.copiedUserId)
                            break
                        end
                    end
                    
                elseif avatarData.action == "RESET" then
                    _G.SharedAvatarData[targetUserId] = nil
                    
                    -- Reset target player ONLY if NOT LocalPlayer
                    for _, player in ipairs(Players:GetPlayers()) do
                        if tostring(player.UserId) == targetUserId and player ~= LocalPlayer then
                            resetSharedAvatar(player)
                            break
                        end
                    end
                end
...
-- Background webhook sync loop
task.spawn(function()
    while task.wait(_G.AvatarWebhookConfig.SyncInterval) do
        if _G.AvatarWebhookConfig.Enabled and _G.CopyAvatarEnabled then
            pcall(processWebhookUpdates)
        end
    end
end)

-- Apply shared avatars to existing players (SKIP LocalPlayer)
local function syncExistingPlayers()
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            local userId = tostring(player.UserId)
            local sharedData = _G.SharedAvatarData[userId]
            
            if sharedData and sharedData.copiedUserId then
                applySharedAvatar(player, sharedData.copiedUserId)
            end
        end
    end
end
...
local function copyAvatarByUsername(username)
    ...
    -- Apply to SELF first
    local success = copyAvatarThroughDescription(userId, LocalPlayer)
    ...
    if success then
        ...
        _G.SharedAvatarData[myUserId] = {
            copiedUserId = userId,
            username = LocalPlayer.Name,
            timestamp = tick()
        }
        
        -- Broadcast to webhook (other users will apply)
        sendAvatarUpdateToWebhook("UPDATE", LocalPlayer.UserId, userId, LocalPlayer.Name)
        
        print("[Avatar] Applied to SELF:", userId)
    ...
    if _G.OriginalAppearance then
        ...
        if character then
            ...
            if humanoid then
                ...
                -- Update shared data
                local myUserId = tostring(LocalPlayer.UserId)
                _G.SharedAvatarData[myUserId] = nil
                
                -- Broadcast reset to webhook (other users will reset)
                sendAvatarUpdateToWebhook("RESET", LocalPlayer.UserId, nil, LocalPlayer.Name)
                
                print("[Avatar] Reset SELF to original")
...
_G.CopyAvatarToggle = Player:Toggle({
    ...
    Desc = "Copy avatars visible to ALL script users",
    Value = false,
    Callback = function(state)
        ...
        if not state then
            ...
            task.spawn(syncExistingPlayers)
        end
    end
})
...
Player:Toggle({
    Title = "Webhook Sync",
    Desc = "Enable Telegram webhook synchronization",
    Value = true,
    Callback = function(state)
        _G.AvatarWebhookConfig.Enabled = state
        if state then
            syncExistingPlayers()
        end
    end
...
Player:Button({
    ...
    Desc = "Reset to original (VISIBLE TO ALL)", 
    Callback = function()
        resetToOriginalAppearance()
    end
})
...
Player:Button({
    ...
    Desc = "Manually sync all avatars from webhook", 
    Callback = function()
        ...
        if WindUI then
            WindUI:Notify({
                Title = "Sync Complete",
                Content = "Avatars synced from webhook!",
                Duration = 2
            })
        end
    end
...
Player:Button({
    Title = "Test Webhook",
    Desc = "Send test message to verify connection", 
    Callback = function()
        local success = sendAvatarUpdateToWebhook("TEST", LocalPlayer.UserId, 0, LocalPlayer.Name)
        
        if WindUI then
            WindUI:Notify({
                Title = success and "✅ Success" or "❌ Failed",
                Content = success and "Webhook connected!" or "Connection failed!",
                Duration = 3
            })
        end
    end
...
Players.PlayerAdded:Connect(function(player)
    ...
    -- Apply shared avatar if exists (SKIP if LocalPlayer)
    if player ~= LocalPlayer then
        task.wait(2)
        local userId = tostring(player.UserId)
        local sharedData = _G.SharedAvatarData[userId]
...
-- Auto reapply on respawn (LocalPlayer)
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(2)
    
    if _G.CopyAvatarEnabled and _G.CurrentCopiedUserId then
        task.spawn(function()
            task.wait(0.5)
            -- Reapply to SELF
            copyAvatarThroughDescription(_G.CurrentCopiedUserId, LocalPlayer)
            print("[Avatar] Reapplied to SELF after respawn")
        end)
    end
end)

-- Monitor other players respawn (NOT LocalPlayer)
Players.PlayerAdded:Connect(function(player)
...
task.spawn(function()
    ...
    if _G.AvatarWebhookConfig.Enabled then
        processWebhookUpdates()
        syncExistingPlayers()
    end
end)
